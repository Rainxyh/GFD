...
class ConvLSTMCell(nn.Module):
    ...
    def init_hidden(self, batch_size, image_size):  # 权重初始化为0表明初始状态对未来完全无先验知识
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        # 确保 `kernel_size` 和 `hidden_dim` 都是具有 len == num_layers 的列表
    ...
    @staticmethod
    def _check_kernel_size_consistency(kernel_size):  # 校验卷积核大小是否符合要求
...
if __name__ == '__main__':
    x = torch.rand((32, 10, 64, 128, 128))
    convlstm = ConvLSTM(input_dim=64, hidden_dim=16, kernel_size=(1, 1), num_layers=1, batch_first=True, bias=True,
